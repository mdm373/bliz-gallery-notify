...
npm run package
$functionCreateResponse = aws lambda create-function --cli-input-json file://./aws/function-create.json --zip-file fileb://./pack/index.zip --role=$role --environment Variables=`{PHONE_NUMBER=$phoneNumber,DEVICE_ID=$deviceId`} | ConvertFrom-Json
